...
def objective(X_1, g_2, X_2, g_3, X_3):
    ...
    return norm(X_1, 'fro')**2 + g_2*norm(np.hstack(X_2), 1) + g_3*norm(tmp, 1)
...
def rpcaADMM(data):
    ...
    N = 3  # the number of matrices to split into (and cost function expresses how you want them)

    A = np.float_(data)    # A = S + L + V
    m, n = A.shape

    g2_max = norm(np.hstack(A).T, np.inf)
    g3_max = norm(A, 2)
    g2 = 0.15*g2_max
    g3 = 0.15*g3_max
    ...
    for k in range(MAX_ITER):
        ...
        async_X3 = pool.apply_async(update, (
            lambda x: x3update(x, B, lambdap, g3, prox_l1, prox_matrix), X_3))
        ...
        zold = z
        z = x + np.tile(-avg(X_1, X_2, X_3) + A*1.0/N, (1, N))
        ...
        h['eps_pri'][k] = np.sqrt(m*n*N)*ABSTOL + RELTOL*np.maximum(norm(x, 'fro'), norm(-z, 'fro'))
        h['eps_dual'][k] = np.sqrt(m*n*N)*ABSTOL + RELTOL*np.sqrt(N)*norm(rho*U, 'fro')

        if (k == 0) or (np.mod(k+1, 10) == 0):
            print('%4d\t%10.4f\t%10.4f\t%10.4f\t%10.4f\t%10.2f' % (
                k+1, h['r_norm'][k], h['eps_pri'][k],
